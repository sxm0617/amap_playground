...
const EventsCoverEventCoverDrag: FC = () => {
  ...
  const handleBind = () => {
    marker.on('dragging', markerDrag)
    polygon.on('dragging', polygonDrag)
    setCoverText('已绑定事件')
  }

  const handleUnbind = () => {
    // 官方提供的off方法无法解绑事件，需要手动更改下面vI属性对应的事件名
    marker.vI.dragging.pop()
    polygon.vI.dragging.pop()
    setCoverText('未绑定事件')
  }
...
export default EventsCoverEventCoverDrag
