...
const EventsMapEventZoom: FC = () => {
  const [map, setMap] = useState<any>()
  const [zoomText, setZoomText] = useState('未绑定事件')
  ...
  const handleBind = () => {
    ...
    map.on('zoomchange', doZoom)
    map.on('zoomend', stopZoom)
    setZoomText('已绑定事件')
  }
  ...
  const handleUnbind = () => {
    ...
    map.vI.zoomstart.pop()
    map.vI.zoomend.pop()
    delete map.vI.zoomchange
    setZoomText('未绑定事件')
  }
...
export default EventsMapEventZoom
